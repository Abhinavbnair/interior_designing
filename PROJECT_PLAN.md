...
## 16. Post-Phase-9 fix: FakeProvider for no-API-key dry runs

Running `scripts/run_phase9_experiments.py` with `LLM_PROVIDER=mock` failed immediately
(`MockProvider ran out of scripted responses`) — expected behavior given `MockProvider`'s design
(a strict test double that only replays pre-scripted responses, used by the test suite), but a
real gap: there was no way to dry-run the harness end-to-end without either an API key or writing
scripted responses by hand.

**Fix:** added `FakeProvider` (`app/llm/provider.py`, `LLM_PROVIDER=fake`) — generates
plausible-but-heuristic responses (keyword matching for extraction, seeded pseudo-random verdicts
for verification) so `run_full_pipeline.py` and `run_phase9_experiments.py` can be smoke-tested
with zero API cost. Logs a warning on first use. 8 new tests (69/69 total passing), including one
that runs the full refinement loop through `FakeProvider` alone with no scripting.

**This does not change the plan or the open gaps in §14** — `FakeProvider` output must never be
reported as a result; it only proves the harness runs without crashing before spending real API
calls on it.

## 17. Next step

Now that `LLM_PROVIDER=fake` unblocks a no-cost dry run of the whole harness, the sequence is:

1. Run `scripts/run_phase9_experiments.py` with `LLM_PROVIDER=fake` and `MAX_CASES=2` to confirm
   the harness runs clean on GPU (it's only been proven offline so far — the diffusion calls
   inside the real conditions A-E have never executed against `FakeProvider`, only against fully
   synthetic PipelineResults in the unit tests).
2. Switch to `LLM_PROVIDER=anthropic` with a real key and repeat with `MAX_CASES=2`. This is the
   single highest-value action — it converts the largest block of unverified work into verified
   work.
3. Scale up test case count and `MAX_CASES` as time allows.

Before the project can be written up as academically complete, §B, §D and §H above still need
real work — particularly §H, since every compliance number depends on the verifier being
trustworthy, and no fake or mock provider can substitute for that check.
